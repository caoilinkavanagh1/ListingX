    ...
    fun setButtonListener(layout: FragmentDonateBinding) {
        layout.donateButton.setOnClickListener {
            ...
            if(totalDonated >= layout.progressBar.max)
               ...
               // val exerciseType = if(layout.exerciseMethod.checkedRadioButtonId == R.id.Direct) "Yoga" else "Pilates"
                totalDonated += amount
                //layout.totalSoFar.text = getString(R.string.total_donated,totalDonated)
                layout.progressBar.progress = totalDonated
                val jobmethod = layout.jobmethod.text.toString() // Retrieve the job method input field value
                val area = layout.area.text.toString() // Retrieve the area input field value
                val username = layout.username.text.toString() // Retrieve the area input field value
                val description = layout.description.text.toString() // Retrieve the area input field value
                donateViewModel.addDonation(DonationModel(paymentmethod = paymentmethod,amount = amount, jobmethod = jobmethod, area = area, username = username, description = description))
            }
        }
    }
